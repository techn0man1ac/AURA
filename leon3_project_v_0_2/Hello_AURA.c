...
/* ПРЯМА АДРЕСАЦІЯ ПАМ'ЯТІ ДЛЯ RENODE-СИМУЛЯЦІЇ */
...
/* ФІКС: Власна автономна реалізація для усунення помилки лінкера undefined reference */
...
unsigned int calculate_block_joint_rgb_entropy(int x0, int y0, int block_w, int block_h) {
    ...
    int row_stride = IMG_WIDTH; 

    for (int y = 0; y < block_h; y++) {
        int global_y_idx = (y0 + y) * row_stride;
...
int main(void) {
    print_str("\n===================================\n");
    print_str("AURA Firmware: Active Renode Execution Core\n");
    print_str("===================================\n\n");
